...
if($_FILES["archivo"]["error"]>0){
  ...
  if(in_array($_FILES["archivo"]["type"], $permitidos) && $_FILES["archivo"]["size"] <= $limite_kb * 1024){
    ...
    if (!file_exists($archivo)) {
      ...
      if ($resultado) {
        $b_foto=mysqli_query($conexion,"SELECT foto_lic FROM usuarios WHERE Id_usuario='$id_piloto'");
        $rfoto=mysqli_fetch_array($b_foto);
        $borra=$rfoto['foto_lic'];
        unlink($borra);

        $insertar=mysqli_query($conexion,"UPDATE usuarios SET foto_lic='$archivo' where Id_usuario='$id_piloto'");
