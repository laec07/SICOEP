...
if ($busca -> num_rows > 1) {//si existe mas de dos
	...
	$inserta_detalle=mysqli_query($conexion,"INSERT combustible_detalle
												SELECT id_solicitud,id_depto,'$id_asignacion',fecha,galones,tipo_combustible,id_precio,precio,total,usuario,codigo_pais,'$asignado','$restante',canal,'$ruta','Ruta Emergente cubre ruta ".$ruta."','','' FROM combustible_detalle WHERE id_solicitud='$id_solicitud' and id_ruta='$id_ruta'
								");
	if ($inserta_detalle) {
		...
		echo"
				<script>
					alert('fallos insercción, verifique!!);
				</script>
