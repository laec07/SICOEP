...
$pendientes_1=mysqli_query($conexion,"SELECT m.estatus, COUNT(m.estatus) as total
                 ...
                 WHERE m.ID IN (SELECT ID
                                FROM (  SELECT MAX(ID) AS id, id_tarea, MAX(fecha_mov) as fecha
                                          FROM mov_tarea 
                                      GROUP BY id_tarea) a)
                AND t.ID=m.ID_tarea AND t.cod_area='$area' AND (m.estatus='PENDIENTE' or m.estatus='REPROGRAMAR' ) GROUP BY m.estatus");
  ...
  $pendientes_2=mysqli_query($conexion,"SELECT t.tarea
                   ...
                   WHERE m.ID IN (SELECT ID
                                  FROM (  SELECT MAX(ID) AS id, id_tarea, MAX(fecha_mov) as fecha
                                            FROM mov_tarea 
                                        GROUP BY id_tarea) a)
                  AND t.ID=m.ID_tarea AND t.cod_area='$area' AND m.estatus='PENDIENTE' GROUP BY m.estatus");
...
$pendientes_3=mysqli_query($conexion,"SELECT m.estatus, COUNT(m.estatus) as total
                 ...
                 WHERE m.ID IN (SELECT ID
                                FROM (  SELECT MAX(ID) AS id, id_tarea, MAX(fecha_mov) as fecha
                                          FROM mov_tarea 
                                      GROUP BY id_tarea) a)
                AND t.ID=m.ID_tarea AND m.estatus='SIN ASIGNAR' GROUP BY m.estatus");
...
<div class="row">
  <div class="col-md-4">
           ...
           <a data-toggle="modal" data-target="#rangoFechas_e" class="btn btn-info">Enviar</a> 
          </div>
</div>
                ...
                $datos=mysqli_query($conexion,"SELECT t.ID as idt, t.tarea, t.fecha_programada, m.ID,t.mensaje, m.estatus, m.fecha_realizado, m.Observaciones, t.cod_area, m.fecha_reprogramado 
                 ...
                 WHERE m.ID IN (SELECT ID
                                FROM (  SELECT MAX(ID) AS id, id_tarea, MAX(fecha_mov) as fecha
                                          FROM mov_tarea 
                                      GROUP BY id_tarea) a)
                AND t.ID=m.ID_tarea AND t.cod_area='$area'  AND (m.estatus='PENDIENTE' or m.estatus='REPROGRAMAR' ) ORDER BY t.ID desc
                LIMIT 100
                ");
                ...
                $datos=mysqli_query($conexion,"SELECT t.ID as idt, t.tarea, t.fecha_programada, m.ID, m.estatus, m.fecha_realizado, m.Observaciones,t.mensaje, t.cod_area, m.fecha_reprogramado 
                 ...
                 WHERE m.ID IN (SELECT ID
                                FROM (  SELECT MAX(ID) AS id, id_tarea, MAX(fecha_mov) as fecha
                                          FROM mov_tarea 
                                      GROUP BY id_tarea) a)
                AND t.ID=m.ID_tarea AND t.cod_area='$area' AND m.estatus='REALIZADO' ORDER BY t.ID desc
                LIMIT 100
                ");
...
<!--inician modales -->
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <form action="tareaexcel.php" method="GET">
  ...
  <!--Finaliza modal rango de fechas ----------------------------------------------------------->
         <!-- /.Movimientos tareas -->
         <div class="modal fade" id="TareaMov">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              ...
              <div class="modal-body">
                <!-- Inicia el data picker -->
                <form action="consultas/guarda_tarea_mov.php" method="POST">
                  ...
                  <div id="fecha" style="display: none;">
                    <div class="form-group">
                      ...
                      <input  type="date" name="date" id="date" placeholder="Fecha Programada" value="<?php echo date("Y-m-d");?>">
                    </div>
                  </div>
                  <div class="form-group">
                            ...
                            <label for="date">Fecha:</label>
                             <div class="input-group date">
                               <div class="input-group-addon">
                                ...
                                <input required="" type="date" name="date" placeholder="Fecha Programada" class="form-control" value="<?php echo date("Y-m-d");?>"> 
                           </div>
                           </div>
                        </div>
                        </div>
                   <button type="submit" class="btn btn-primary pull-rigth">Procesar</button>
                </form>
